...
export function fightingInput(command:fightingInputCommand,inputs:string[],flip:boolean=false):boolean{
    ...
    if(flip){
        directions=command.directions.split('').map((v)=>(XflipList[v]??v)).join('');
    }
            ...
            if(dirStr.length>=directions.length){
                ...
                for(let kk=0;kk<dirStr.length-directions.length+1;kk++){
                    for(let k=kk;k<dirStr.length;k++){
                        if(dirStr[k]===directions[passed] || nextTo[dirStr[k]]?.includes(parseInt(directions[passed]))){
                            passed++;
                        }else{
                            if(dirStr[k]==='5' || passed>0 && dirStr[k]===directions[passed-1] || nextTo[dirStr[k]]?.includes(parseInt(directions[passed-1]))){
                                continue;
                            }
                            break
                        }
...
export function fightingInputMulti(commands:Record<string,fightingInputCommand>,inputs:string[],flip:boolean=false):Record<string,boolean>{
